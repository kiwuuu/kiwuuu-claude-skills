> NOTE (2026-07-21): reference file merged from the retired `shadcn-ui` skill. Its `tailwind-theme-builder` prerequisite applies to Vite projects only — for Next.js, use the setup in nextjs-shadcn's SKILL.md instead. Read this file for the gotchas (Radix Select empty string, RHF null spread, Dialog max-width override, Lucide tree-shaking).
