...
**Match the project's existing package manager — check the lockfile first** (`package-lock.json` → npm, `pnpm-lock.yaml` → pnpm, `bun.lock`/`bun.lockb` → bun). Never mix managers in one repo (Limonella and the T1 services use npm).

For brand-new projects with no lockfile, default to bun:
- `bun install` / `bun add` / `bunx --bun` (instead of npx)
